...
class FileMoveProcessingThread(QThread):
    progress = Signal(int)
    error_signal = Signal(list)
    result_signal = Signal(list)
    def __init__(self,parent, excelDir, fileDir):
    ...
    def movImage(self):
        ...
        for i,fileID in enumerate(self.fileID_list):
            ...
            self.progress.emit(progress_percentage)                         # 更新进度条信号
            ...
            for i, (_ , row) in enumerate(dpSingleFile.iterrows()):   
                ...
                nPDFpath = os.path.join(os.path.dirname(self.fileDir),'PDF')
                self.generatePDF(fileInternalpath,nPDFpath,PDFname)         # 生成PDF文件
...
class PDFMoveProcessingThread(FileMoveProcessingThread):
    progress = Signal(int)
    error_signal = Signal(list)
    result_signal = Signal(list)
    def __init__(self,parent, excelDir, fileDir):
        super().__init__(parent, excelDir, fileDir)
   
    def movePDF(self):
        total_files = len(self.fileID_list)
        for i,fileID in enumerate(self.fileID_list):

            progress_percentage = round(((i+1) / total_files) * 100)# 设置进度
            self.progress.emit(progress_percentage)                         # 更新进度条信号
            ...
            for i, (_ , row) in enumerate(dpSingleFile.iterrows()):   
                ...
                fileExternal_name = row.loc['案卷级档号']
                fileInternal_name = row.loc['文件级档号']
                InternalDir_path = os.path.join(os.path.dirname(self.fileDir),'PDF',fileExternal_name)
                if not os.path.exists(InternalDir_path):
                    os.makedirs(InternalDir_path)
                is_last_file = False
                ...
                PDFname = fileInternal_name + '.pdf'
                # fileInternalpath = os.path.join(self.fileDir, fileExternal_name, fileInternal_name)
                PDFpath = os.path.join(os.path.dirname(self.fileDir),'PDF',PDFname)
                
                # nPDFpath = os.path.join(os.path.dirname(self.fileDir),'PDF', fileExternal_name)
                nPDFpath = os.path.join(os.path.dirname(self.fileDir),'PDF',fileExternal_name,PDFname)
                shutil.move(PDFpath, nPDFpath)
                if is_last_file:
                    continue
...
if __name__ == '__main__':
    fb = FileMoveProcessingThread("卷内目录（给档案馆用）2022年度.xlsx",
